...
import {
    ERoleType, TMinMaxArray, TGroupId,
    EMeasureRole, IMetadata, IData, TGroupDataRow, TMeasureRoles, IVisualGroup
} from '../../models';
import PowerBiUtils from './utils';

export class PowerBIDataParser {
    private powerBiData: powerbi.DataView;
    /* private getter values */
    private _metadata: IMetadata;
    private _dataset: IData;
    ...
    updateData(rawData: powerbi.DataView) {
        this.initConfig();
        ...
        this._dataset = {
            groups: [],
            categories: [],
            groupMapping: new Map<TGroupId, TGroupDataRow[]>(),
            minMaxMapping: new Map<TGroupId, TMinMaxArray>()
        };
    ...
    private collectMeasures(measureColumns: powerbi.DataViewMetadataColumn[]) {
        let hasActualMeasure = false;
        let actualMeasureCount = 1;
        const prevPushedRoles = new Map<TMeasureRoles, number>();
        measureColumns.forEach(measureColumn => {
            ...
            const measureRoles = Object.keys(measureColumn.roles).map(roleKey => {
                ...
                if (roleKey === ERoleType.AC && hasActualMeasure) {
                    roleType = `AC${actualMeasureCount}` as TMeasureRoles;
                    actualMeasureCount++;
    ...
    private collectGroups(groupColumns: powerbi.DataViewHierarchyLevel[]) {
    ...
    private collectRows(rowColumn: powerbi.DataViewMetadataColumn) {
    ...
    private gatherDataGroups() {
        const columnsCount = this._metadata.columns.length;
        const groups = PowerBiUtils.getVisualGroup(
            columnsCount,
            this.powerBiData.matrix.columns.root.children
        );
    ...
    private collectGroupRows(
        rowId: string,
        group: IVisualGroup,
        row: powerbi.DataViewMatrixNode,
        rowIndex: number
    ) {
        ...
        let dataRow: TGroupDataRow = { rowId };
        group.valueHandlers.forEach((valueIndex, roleIndex) => {
            dataRow[this._metadata.roles[roleIndex]] = row.values[valueIndex].value as number;
        });
